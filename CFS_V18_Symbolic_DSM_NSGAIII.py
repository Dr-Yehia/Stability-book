...
def conservative_calibration(y_true: Any, y_pred: Any, target_bias: tuple[float, float] = (1.03, 1.05)) -> dict[str, float]:
    ...
    yp = np.asarray(y_pred, dtype=float)
    records = []
    for k in np.round(np.arange(0.94, 1.0301, 0.005), 3):
...
def run_pysr_stage(
    ...
    cols = features
    train_df = data.dropna(subset=cols + [PYSR_TARGETS[target_name]]).copy()
...
def main() -> int:
    ...
    if args.run_pysr:
        for target_name in PYSR_TARGETS:
            ...
            eqs = run_pysr_stage(symbolic, target_name, out_dir, args.preset, features)

#!/usr/bin/env python3
"""
CFS v11 — Two Key Innovations vs v10
═══════════════════════════════════════════════════════════════
CHANGE 1: Remove Winsorize completely (was clipping FM=F targets → 107% error)
CHANGE 2: DSM Residual Learning for FM=F group
          Instead of predicting Pt/Py directly, the model learns the
          deviation from DSM theoretical value:
            residual_F = Pt/Py - Pne/Py
          This anchors the model on physics, drastically reducing FM=F error.

All other v10 features retained:
  - G5 split (G5a_C2C_F / G5b_C2C_Other / G5c_C2C_LF)
  - Target encoding after split
  - early_stopping_rounds = 300
  - Huber loss
  - Ridge blend per group (threshold +0.002)
  - is_F + Pne_x_isF features
"""
...
# ── INNOVATION 2: DSM physics anchor for FM=F ──
# Pne/Py from DSM column curve (theoretical flexural buckling)
def dsm_global(lc):
    lc = np.asarray(lc, dtype=float)
    return np.where(lc <= 1.5,
                    0.658 ** (lc**2),
                    0.877 / (lc**2))

df["DSM_global"] = dsm_global(df["λc"])
# Residual: how much does actual Pt/Py deviate from DSM theory?
df["residual_F"] = df["Pt/Py"] - df["DSM_global"]
# Ratio: actual / theoretical (useful feature for non-F modes too)
...
BASE_FEATURES = [
    ...
    "is_F", "Pne_x_isF",
    "DSM_global", "Pt_DSM_ratio",   # NEW in v11
...
print("═"*60)
# v11 FIX: NO Winsorize — use raw targets
y_tr_w = y_tr_np.copy()  # no clipping
...
# ═══════════════════════════════════════════════════════════
# 7. PHASE 1: SPECIALIST MODELS
#    INNOVATION: G5a_C2C_F uses Residual Learning
# ═══════════════════════════════════════════════════════════
print("\n" + "═"*60)
print("  PHASE 1: Specialist Models (G5a_C2C_F → Residual Learning)")
print("═"*60)
...
for g in groups:
    ...
    # ── INNOVATION: Residual Learning for G5a_C2C_F ──
    if g == "G5a_C2C_F":
        print(f"  → Using DSM Residual Learning for FM=F")
        yg_res_tr = res_tr_np[tr_mask]   # train on residual = Pt/Py - DSM
        dsm_g_te  = dsm_te[te_mask] if n_te > 0 else np.zeros(1)

        oof_res, pte_res, r2_res = train_group(g, Xg_tr_g, yg_res_tr, Xg_te_g)
        # Convert residual predictions back to Pt/Py
        dsm_g_tr = df_tr_raw.loc[tr_mask, "DSM_global"].values
        oof_g  = oof_res + dsm_g_tr
        pte_g  = pte_res + dsm_g_te if n_te > 0 else np.zeros(1)

        yg_tr_g = y_tr_np[tr_mask]  # actual Pt/Py for comparison
...
plt.tight_layout()
plt.savefig("v11_scatter.png", bbox_inches="tight")
plt.show()
print("✅  v11_scatter.png saved")
...
plt.tight_layout()
plt.savefig("v11_shap.png", bbox_inches="tight")
plt.show()
...
}).to_csv("v11_predictions.csv", index=False)
print("✅  v11_predictions.csv saved")

print(f"\n{'═'*62}")
print(f"  🎯  v11 COMPLETE")

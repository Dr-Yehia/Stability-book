...
4. Optionally run PySR in staged mode to discover candidate ``g_symbolic``
   equations for actual / teacher / hybrid correction targets.
...
---------------------
By default, PySR training excludes the official holdout rows; holdout rows are
reserved for final equation evaluation only.

Excluded from symbolic equations by design:
``StudyID``, ``TestLabel``, true failure mode, fastener spacing, row labels, and
any target-derived features other than the explicit training target.
"""
...
# Publication-safe default: PySR must NOT train on the official holdout rows.
# Holdout rows are reserved for final equation evaluation only.
...
def conservative_calibration(y_true: Any, y_pred: Any, target_bias: tuple[float, float] = (1.03, 1.05)) -> dict[str, float]:
    ...
    yp = np.asarray(y_pred, dtype=float)
    records = []
    # Conservative design calibration should not increase predictions.
    # k <= 1.0 reduces unsafe overprediction while preserving ranking.
...
def run_pysr_stage(
    ...
    # by default, symbolic equations are trained only on non-holdout rows.
    # The official holdout is used only in evaluate_equations().
    if "is_holdout" in data.columns and not allow_holdout_training:
        train_source = data[~data["is_holdout"].astype(bool)].copy()
    else:
        train_source = data.copy()

    train_df = train_source.dropna(subset=cols + [PYSR_TARGETS[target_name]]).copy()
    if len(train_df) < MIN_PYSR_TRAIN_ROWS:
        raise ValueError(
            f"Not enough non-holdout training rows for target={target_name}: "
            f"{len(train_df)} rows available. This usually means full V17 teacher "
            "inference was unavailable; do not train teacher/hybrid equations on "
            "holdout-only predictions. Run without --skip-teacher or use target=actual only."
        )

    x_train = train_df[cols].astype(float)
    y_train = train_df[PYSR_TARGETS[target_name]].astype(float)
    ...
    equations.insert(2, "train_rows", int(len(train_df)))
    equations.insert(3, "trained_on_holdout", bool(allow_holdout_training))
...
def expression_to_callable(expression: str, features: list[str]):
    ...
    # Also support PySR/exported equations that use x0, x1, ... instead of feature names.
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Research/debug only. Allows PySR to train on official holdout rows. Do not use for publication metrics.",
    )
...
def main() -> int:
    ...
    if args.run_pysr:
        for target_name in PYSR_TARGETS:
            ...
            try:
                ...
                print(f"[WARN] Skipping PySR target={target_name}: {exc}")
    ...
    report = {
        ...
        "data": {
            ...
            "pysr_trains_on_holdout": bool(args.allow_holdout_training),
            "n_symbolic_train_non_holdout": int((~symbolic["is_holdout"].astype(bool)).sum()) if "is_holdout" in symbolic.columns else int(len(symbolic)),
        },
